...
export function createFamilyPhonePairingsRepo(db: DatabaseClient): FamilyPhonePairingsRepo {
  const insertStmt = db.prepare<FamilyPhonePairingRow, [string, number, string, string, string]>(
    `INSERT INTO family_phone_pair_requests (user_code, user_id, label, kind, expires_at)
     VALUES (?, ?, ?, ?, ?)
     RETURNING id, user_code, user_id, label, kind,
               created_at, expires_at, consumed_at`,
  );
  const findByUserCodeStmt = db.prepare<FamilyPhonePairingRow, [string]>(
    `SELECT id, user_code, user_id, label, kind,
            created_at, expires_at, consumed_at
